...
package body Field.Operations is
  ...
  -- Row around field makes things easier
  subtype Row_Id    is Integer
      range Valid_Row'First    - 1 .. Valid_Row'Last    + 1;
  subtype Column_Id is Integer
      range Valid_Column'First - 1 .. Valid_Column'Last + 1;

  -- Possible states of a cell
  type State_Id is (Normal, Marked, Stepped_On);

  -- Extra value means not yet counted
  subtype Count_Value is Integer
      range Valid_Count'First - 1 .. Valid_Count'Last;
  ...
  -- A mine field
  type Field_Set is array (Row_Id, Column_Id) of Cell_Info;
  ...
  begin
    ...
    if Mine_Field (Cell.Row, Cell.Column).State = Marked then
      -- Can't count a marked cell
      return;
    end if;

    if Mine_Field (Cell.Row, Cell.Column).Count not in Valid_Count then
      -- Cell has not been counted
    ...
    Count_Rows : for Row in Cell.Row - 1 .. Cell.Row + 1 loop
      Count_Columns : for Column in Cell.Column - 1 .. Cell.Column + 1 loop
        if Mine_Field (Row, Column).Mine then
          Mine_Field (Cell.Row, Cell.Column).Count :=
              Mine_Field (Cell.Row, Cell.Column).Count + 1;
        end if;
      end loop Count_Columns;
    end loop Count_Rows;

    User_If.Display_Count (Count   => Mine_Field (Cell.Row, Cell.Column).Count,
                           Stepped => Mine_Field (Cell.Row, Cell.Column).State
                                    = Stepped_On,
                           Cell    => Cell);
    end if;
  end Detect;
  ...
  procedure Reset is
    subtype Rand_Set_Index is Integer
       range 1 .. Valid_Row'Last * Valid_Column'Last;
    -- For randomly placing mines
    type Rand_Set is array (Rand_Set_Index) of Cell_Location;
  ...
  begin
    Dead := False;
    Mine_Field := Field_Set'(others => (others =>
        Cell_Info'(State   => Normal,
        Mine    => False,
    ...
    Fill_Rows : for Row in Valid_Row loop
      Fill_Columns : for Column in Valid_Column loop
        Rand_List (Valid_Column'Last * (Row - 1) + Column) :=
            Cell_Location'(Row => Row, Column => Column);
    ...
    Display_Rows : for Row in Valid_Row loop
      Display_Columns : for Column in Valid_Column loop
        if Row    = Valid_Row'First
        or else Row    = Valid_Row'Last
        or else Column = Valid_Column'First or else Column = Valid_Column'Last
        then
          -- Cell is on the edge; automatically count these for the player
  ...
  function Stepped_On_Neighbor (Cell : Cell_Location) return Boolean is
  begin
    Check_Row : for Row in Cell.Row - 1 .. Cell.Row + 1 loop
      Check_Column : for Column in Cell.Column - 1 .. Cell.Column + 1 loop
        ...
        and then Mine_Field (Row, Column).State = Stepped_On then
          return True;
        end if;
  ...
  -- See if a cell has a marked neighbor
  function Marked_Neighbor (Cell : Cell_Location) return Boolean is
  begin
    Check_Row : for Row in Cell.Row - 1 .. Cell.Row + 1 loop
      Check_Column : for Column in Cell.Column - 1 .. Cell.Column + 1 loop
        if (Row /= Cell.Row or else Column /= Cell.Column)
        and then Mine_Field (Row, Column).State = Marked then
          return True;
        end if;
      end loop Check_Column;
    end loop Check_Row;
  ...
  procedure Auto_Step (Cell : in Cell_Location) is
    ...
    Step_Rows : for Row in Cell.Row - 1 .. Cell.Row + 1 loop
      if Row in Valid_Row then
        Step_Columns : for Column in Cell.Column - 1 .. Cell.Column + 1 loop
          ...
          and then Mine_Field (Row, Column).State /= Marked then
            if Cell_Satisfied
            or Mark_Count_Satisfied ( (Row => Row, Column => Column) ) then
  ...
  begin
    ...
    if Stepped_On_Neighbor (Cell) or else Marked_Neighbor (Cell) then
      -- Force detect to count cell's neighbors
      Mine_Field (Cell.Row, Cell.Column).State := Marked;

      -- Automatically detect around marked cell
      Count_Rows : for Row in Cell.Row - 1 .. Cell.Row + 1 loop
        if Row in Valid_Row then
          Count_Columns : for Column in Cell.Column - 1 .. Cell.Column + 1 loop
      ...
      case Old_State is
        ...
        when Marked =>
          ...
          User_If.Display_Count (
              Count   => Mine_Field (Cell.Row, Cell.Column).Count,
              Stepped => Mine_Field (Cell.Row, Cell.Column).State = Stepped_On,
              Cell    => Cell);

          To_Mark := To_Mark + 1;
        when Stepped_On =>
          -- Can't marked a stepped-on cell
          null;
      end case;
    ...
    begin
      Count_Rows : for Row in Cell.Row - 1 .. Cell.Row + 1 loop
        if Row in Valid_Row then
          Count_Columns : for Column in Cell.Column - 1 .. Cell.Column + 1 loop
            if Column in Valid_Column
            and then Mine_Field (Row, Column).State = Normal then
              Result := Result + 1;
            end if;
          end loop Count_Columns;
        end if;
  ...
  begin
    ...
    if not Stepped_On_Neighbor (Cell) and then not Marked_Neighbor (Cell) then
      ...
      -- Automatically detect around stepped-on cell
      Count_Rows : for Row in Cell.Row - 1 .. Cell.Row + 1 loop
        if Row in Valid_Row then
          Count_Columns : for Column in Cell.Column - 1 .. Cell.Column + 1 loop
            if Column in Valid_Column then
      ...
      if Mine_Field (Cell.Row, Cell.Column).Mine then
        -- Stepped on a mine!
        Dead := True;
      ...
      if User_If.Auto_Marking then
        -- See if stepping here has created any normal cells that obviously
        --  contain mines;
        --  if so, mark them.
        if Mine_Field (Cell.Row,
                       Cell.Column).Count - Num_Marked_Neighbors (Cell)
            = Num_Normal_Neighbors (Cell) then
  ...
  begin
    if Dead then
      -- A mine has been stepped on
      return Lost;
    end if;

    Check_Rows : for Row in Valid_Row loop
      Check_Columns : for Column in Valid_Column loop
        if Mine_Field (Row, Column).State = Normal
        or else (Mine_Field (Row, Column).State = Marked)
              /= Mine_Field (Row, Column).Mine then
          return In_Progress;
        end if;
