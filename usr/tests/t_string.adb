...
begin
  ...
  and then Argument.Get_Parameter = "-a" then
    ...
    declare
      ...
      N5 : constant String := N4 & "34";
      L1 : constant Natural := Str_Util.Locate (N5, "34", Forward => True);
      L2 : constant Natural := Str_Util.Locate (N5, "34", Occurence => 2,
                                                Forward => False);
    begin
      Basic_Proc.Put_Line_Output ("Image " & N4);
      Basic_Proc.Put_Line_Output ("Append 34 and locate 34, 1 forwd:" &
        Natural'Image (L1) & ", and 2 backwd:" & Natural'Image (L2));
    end;
