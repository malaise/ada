...
package body Table is
  ...
  Start_Purge : constant := 57; Stop_Purge : constant := 63;
  Start_Undo  : constant := 67; Stop_Undo  : constant := 72;
  Start_Redo  : constant := 76; Stop_Redo  : constant := 81;
  ...
  begin
    ...
    -- Compute offset of play stacks and pool and done stacks and put them
    Done_Pool_Y := (Menu_Row + 1) * Font_Height + Y_Gap;
    Play_Y := Done_Pool_Y + Cards.Deck.Height + Y_Gap;
    ...
    for I in Cards.Dock_Range loop
      Cards.The_Xdock(I).Move (Dock_Of (I));
      Cards.The_Xdock(I).Show (True);
      Cards.The_Xdock(I).Do_Raise;
    end loop;
  ...
  begin
    Console.Set_Name ("Klondike " & "(" & Int_Img (Num) & ")");
  end Set_Game_Num;
  ...
  begin
    return (X => Play_X + (Deck.Suit_List'Pos (Suit) + 3)
                          * (Deck.Width + X_Gap),
            Y => Done_Pool_Y);
  end Done_Of;

  -- Position (X, Y) of card within a DOck stack
  function Dock_Of (Dock : Cards.Dock_Range) return Deck.Position_Rec is
  begin
    return (X => Play_X + (Dock - 1) * (Deck.Width + X_Gap),
            Y => Done_Pool_Y);
  end Dock_Of;
