...
package body Cards is
  ...
  begin
    ...
    -- Create the playing and dock stacks
    ...
    for I in Dock_Range loop
      The_Xdock(I).Create_Empty (Play_Stack_Range'Last + I, Squared => True);
      The_Dock(I).Xcard := The_Xdock(I)'Access;
      The_Dock(I).Suit := Deck.Empty;
      The_Dock(I).Name := I;
      The_Dock(I).Image := "o" & Normal (I, 1);
    end loop;
  ...
  begin
    ...
    if Acc.Is_Card then
      ...
      -- Dock stack
      return The_Dock(Acc.Get_Name - Play_Stack_Range'Last)'Access;
    end if;
  end X_To_Card;
