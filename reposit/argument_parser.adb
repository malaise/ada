    ...
    if Str'Length >= 1 and then Str(1) = '-' then
      ...
      if Str'Length >= 2 and then Str(2) = '-' then
        ...
        if Len = 3 then
          ...
          P_Dscr.Error := As.U.Tus ("Argument " & Str & " at pos "
             & Integer_Image(Arg_No) & " is not valid");
          return;
        end if;
        ...
        -- Not found
        P_Dscr.Error := As.U.Tus ("Argument " & Str & " at pos "
           & Integer_Image(Arg_No) & " is not expected");
        return;
      end if;
      ...
      if Str'Length = 2 then
        ...
        for I in 2 .. Str'Last loop
          ...
          if not Found then
            P_Dscr.Error := As.U.Tus ("Argument " & Str & " at pos "
               & Integer_Image(Arg_No) & " has not expected key " & Str(I));
            return;
          end if;
      ...
      if Arg.Index /= 0 then
        ...
        if Dscr.First_Occurence(Arg.Index) = 0 then
          ...
          Dscr.Error := As.U.Tus ("Argument "
             ...
             & " at pos "
             & Integer_Image(I) & " appears several times");
          return Dscr;
        end if;
            ...
            if Arg.List.Element (J) = The_Keys(K).Key_Char then
              if Dscr.First_Occurence(K) = 0 then
                ...
                Dscr.Error := As.U.Tus ("Argument "
                   & Argument.Get_Parameter (Occurence => I)
                   & " at pos "
                   & Integer_Image(I) & " makes key " & The_Keys(K).Key_Char
