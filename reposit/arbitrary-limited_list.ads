...
package Arbitrary.Limited_List is
  ...
  subtype Arb_Natural  is Number;
  subtype Arb_Positive is Number;
  ...
  function List_Length (List : List_Type) return Arb_Natural;
...
private
  ...
  type List_Type is limited new Ada.Finalization.Limited_Controlled with record
    ...
    In_Cb     : Boolean := False;
    Pos_First : Arb_Natural := Zero;
    Pos_Last  : Arb_Natural := Zero;
    Current   : Link    := null;
    First     : Link    := null;
    Last      : Link    := null;
  end record;
