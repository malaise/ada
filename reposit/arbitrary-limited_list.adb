...
package body Arbitrary.Limited_List is
  ...
  -- "Distance between 2 naturals
  function Dist (A, B : Arb_Natural) return Arb_Natural is
  ...
  begin
    ...
    if From_Current then
      ...
      case Where is
        when Next =>
          if Dist (Number, List.Pos_First) > List.Pos_First / Two then
            New_Pos := List.First;
            New_Pos_First := One;
        ...
        when Prev =>
          if Dist (Number, List.Pos_Last) > List.Pos_Last / Two then
            New_Pos := List.Last;
  ...
  procedure Permute (List         : in out List_Type;
                     Number1      : in Arb_Natural;
                     Number2      : in Arb_Natural;
                     Where        : in Direction := Next;
                     From_Current : in Boolean   := False) is
    Current_Position : constant Arb_Positive
                     := Get_Position (List);
    Link1, Link2 : Link;
  begin
      ...
      case Where is
        when Next =>
          while I <= Occurence loop
            Found := False;
            if I /= One or else From = Skip_Current then
  ...
  begin
    ...
    if Match /= null then
      Found := Search_Match (List, Match, Criteria, Where, One, From);
    else
    ...
    while Found loop
      ...
      if Match /= null then
        Found := Search_Match (List, Match, Criteria, Where, One, Skip_Current);
      else
  ...
  begin
    ...
    declare
      ...
      procedure Quick (Left, Right : in Arb_Positive) is
        -- Middle of the slice
        I_Frontier : constant Arb_Positive := (Left + Right) / Two;
        L_Frontier : Link;
        -- Indexes in both halfs of the slice
        I_Left, I_Right : Arb_Positive;
        L_Left, L_Right : Link;
     begin
        I_Left := Left;
        I_Right := Right;
