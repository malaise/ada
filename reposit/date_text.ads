...
package Date_Text is
  ...
  -- %D : day number in month on 1 or 2 digits
  -- %H : hours on 2 digits
  ...
  -- - the "%B" or %D sequence is immediately followed by another "%x" sequence
  --   (except "%%") or by a lower case letter, because the scanning of a full
  --   month name or day (on 1 or 2 digits) would not be possible

  -- Compute the length of text generated by a format
  -- Raise Invalid_Format if the format is not valid
  -- Raise Unknown_Length if the format is valid but refers to %B or %D
  --  (variable length)
